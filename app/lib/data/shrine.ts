...
export async function fetchShrinesByLocation(lat: string, lng: string) {
  if (!isValidLatLng(lat, lng)) {
    return [] as Shrine[];
  }

  try {
    const params = new URLSearchParams({ searchType: "List" });
    params.set("lat", lat);
    params.set("lng", lng);
    ...
    if (isRedirectError(error)) throw error;
    console.error("[data/shrine] fetchShrinesByLocation error:", error);
    await setFlash({ type: "error", message: "リクエストに失敗しました" });
    return [] as Shrine[];
  }
}
